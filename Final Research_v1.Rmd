...
CPI_ppa_cleaned <- CPI_ppa_raw %>%
  select(Date = TIME_PERIOD, `Percent per annum` = OBS_VALUE) %>%   # Rename columns for clarity
  mutate(Date = paste0(Date, "-01"),                                # Add day to date string
         Date = ymd(Date)) %>%                                      # Convert to Date format
  arrange(Date)
...
```{r}
# Merge CPI_index with CPI_ppa
CPI_combined <- inner_join(CPI_index_cleaned, CPI_ppa_cleaned, by = "Date")

# Merge that with merged energy + PPI data from EDA Code Notebook
